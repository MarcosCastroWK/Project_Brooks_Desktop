...
namespace LibSILC
{
    public class clsUnidadeDados
    {
        ...
        public clsUnidades PegaDados(clsUnidades pUnidades, int pCodigo)
        {
            try
            {
                ConectaBanco();
                ...
                if (l_ds.Tables.Count > 0)
                {
                    ...
                    if (l_dt.Rows[0]["Codigo"].ToString() != "")
                        pUnidades.Codigo = Convert.ToInt32(l_dt.Rows[0]["Codigo"]);
                    pUnidades.Descricao = l_dt.Rows[0]["Descricao"].ToString();
                }
                return pUnidades;
            }
            catch (Exception ex)
            {
                return new clsUnidades();
            }
            finally
            {
                DesconectaBanco();
        ...
        public DataTable PegaDados(clsUnidades pUnidades, int pCodigo, bool pUltimoRegistro)
        {
            try
            {
                ConectaBanco();
        ...
        public void Inserir(clsUnidades pUnidades, int pCodigo = 0)
        {
            try
            {
                s = "";
                s = s + "insert into Unidades \n";
                s = s + "( \n";
                if (pCodigo > 0)
                    s = s + "  Codigo, \n";
                s = s + "  Descricao \n"; 
        ...
        public string Alterar(clsUnidades pUnidades, int pCodigo)
        {
            ConectaBanco();
            MySqlCommand command = oDB.MySqlConnect.CreateCommand();
            MySqlTransaction transaction;
            ...
            try
            {
                s = "";
                s = s + "update Unidades \n";
                s = s + "set Descricao = '" + pUnidades.Descricao + "' \n";
                s = s + "where Codigo = " + pCodigo + " \n";        
                command.CommandText = s;
                command.ExecuteNonQuery();
                transaction.Commit();
