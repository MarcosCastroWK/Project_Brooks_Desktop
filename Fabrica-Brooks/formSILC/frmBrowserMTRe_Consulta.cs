...
namespace formSILC
{
    public partial class frmBrowserMTRe_Consulta : Form
    {
        public LibSILC.clsSenhaIPMDados oSenhaIPM = new LibSILC.clsSenhaIPMDados();

        public frmBrowserMTRe_Consulta()
        {
            InitializeComponent();
        }
        ...
        private void butConsultaNF_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                _Url = "http://mtr.ima.sc.gov.br/ControllerServlet?acao=relatorio&nomeRelatorio=manifesto&manifesto=" + lblNumeroMTRe.Text + "&condicao=N";
                using (var client = new WebClient())
                {
                    client.DownloadFile(_Url, lblNumeroMTRe.Text + ".pdf");
                }
            }
            finally
            {
                System.Diagnostics.Process.Start(lblNumeroMTRe.Text + ".pdf");
            }
        }
