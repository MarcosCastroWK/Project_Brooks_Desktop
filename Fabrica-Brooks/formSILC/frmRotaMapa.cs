...
namespace formSILC
{
    public partial class frmRotaMapa : Form
    {
        ...
        {            
            ...
            foreach (DataRow _dr in _dt.Rows)
            {
                if (_dr.RowState.ToString() != "Deleted")
                {
                    ...
                    for (int i = 0; i < _dt.Columns.Count; i++)
                    {
                        ...
                        if (i == 36)
                        {
                            if (_dr2[36].GetType() != Type.GetType("System.String")) 
                                _dr2[36] = 0;
                        }
                    }
            ...
            //Grade.Columns["NovaLinha"].SortMode = DataGridViewColumnSortMode.NotSortable;
        }

        private void frmRotaMapa_Load(object sender, EventArgs e)
        {
            if (_dt.Columns.Count <= 34)
            {
        ...
        private void EstiloGrades(DataGridView oGrade)
        {
            ...
            oGrade.Columns["Solicitante"].SortMode = DataGridViewColumnSortMode.NotSortable;
            oGrade.Columns["Data"].Width = 70;
            oGrade.Columns["Data"].SortMode = DataGridViewColumnSortMode.NotSortable;
            oGrade.Columns["Hora"].Width = 50;

            oGrade.Columns["NumeroMTRe"].SortMode = DataGridViewColumnSortMode.NotSortable;
            oGrade.Columns["DataProgramada"].SortMode = DataGridViewColumnSortMode.NotSortable;
            oGrade.Columns["Quantidade"].SortMode = DataGridViewColumnSortMode.NotSortable;
            oGrade.Columns["HoraProgramada"].SortMode = DataGridViewColumnSortMode.NotSortable;
            oGrade.Columns["Map"].SortMode = DataGridViewColumnSortMode.NotSortable;
            oGrade.Columns["DestinoFinal"].SortMode = DataGridViewColumnSortMode.NotSortable;
            oGrade.Columns["Observacao"].SortMode = DataGridViewColumnSortMode.NotSortable;

            oGrade.Columns["NomeFantasiaCliente"].Width = 200;
            oGrade.Columns["NomeFantasiaCliente"].HeaderText = "Cliente";
            oGrade.Columns["NomeFantasiaCliente"].SortMode = DataGridViewColumnSortMode.NotSortable;
            ...
            oGrade.Columns["ExecutarServico"].SortMode = DataGridViewColumnSortMode.NotSortable;
            ...
            oGrade.Columns["ModeloCaminhao"].SortMode = DataGridViewColumnSortMode.NotSortable;

            oGrade.Columns["NomeMotorista"].HeaderText = "Motorista";
            oGrade.Columns["NomeMotorista"].Width = 100;
            oGrade.Columns["NomeMotorista"].SortMode = DataGridViewColumnSortMode.NotSortable;

            oGrade.Columns["Unidade"].HeaderText = "Un";
            oGrade.Columns["Unidade"].Width = 25;
            oGrade.Columns["Unidade"].SortMode = DataGridViewColumnSortMode.NotSortable;

            oGrade.Columns["CidadeBairroEndereco"].HeaderText = "Cidade-Bairro-Endereço";
            oGrade.Columns["CidadeBairroEndereco"].Width = 330;
            //oGrade.Columns["CidadeBairroEndereco"].SortMode = DataGridViewColumnSortMode.NotSortable;
        }
        ...
        {
            string _sortColumn = "";
            if (Grade.SortedColumn != null)
                _sortColumn = Grade.SortedColumn.Name;
            int iDesaf = 1;
            if (Grade.SortOrder == SortOrder.Ascending)
            {
                foreach (DataRow _dra in _dtProgramadosNaRota.Select("", "CidadeBairroEndereco"))
                {
                    _dra["NovaLinha"] = iDesaf;
                    iDesaf++;
                }
            }
            else if (Grade.SortOrder == SortOrder.Descending)
            {
                foreach (DataRow _dra in _dtProgramadosNaRota.Select("", "CidadeBairroEndereco desc"))
                {
                    _dra["NovaLinha"] = iDesaf;
                    iDesaf++;
                }
            ...
            {
                foreach (DataRow _dra in _dtProgramadosNaRota.Rows)
                {
                    _dra["NovaLinha"] = iDesaf;
                    iDesaf++;
                }
        ...
        private void NumeraDataTableConformeGrade()
        {
            //for (int ix = 0; ix < Grade.Rows.Count - 1; ix++)
            //{
            //    DataGridViewRow _dgr = Grade.Rows[ix];
            //    DataRow _dr = _dtProgramadosNaRota.Rows[ix];
            //    if (_dgr.Cells["NovaLinha"].Value.ToString() != "")
            //        _dr["NovaLinha"] = Convert.ToInt32(_dgr.Cells["NovaLinha"].Value);
            //}
            //bindingSource.DataSource = _dtProgramadosNaRota;
            //Grade.DataSource = bindingSource.DataSource;
            //Grade.Refresh();
        ...
        private void pd_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            int _NrLinhas = 620;
            //NumeraDataTableConformeGrade();
            DataRow[] _drArrayRota = _dtProgramadosNaRota.Select("", "NovaLinha asc");

            bool bJaImprimiuRodape = false;
            if (_ColunaOrdem == 0)
            {
                foreach (DataGridViewColumn _dc in Grade.Columns)
                {
                    if (_dc.Name == "NovaLinha" || _dc.DataPropertyName == "Ordem")
                    {
                        _ColunaOrdem = _dc.Index;
                        break;
                    }
                    _ColunaOrdem++;
                }
                Grade.Sort(Grade.Columns["NovaLinha"], ListSortDirection.Ascending);
            }
            for (int i = regs; i <= _drArrayRota.Length - 1; i++)
            {
                DataRow drRota = _drArrayRota[i];
                if (ln < _NrLinhas)
                {
                    ln = ln + 12F;
                    cl = 10.0F;
                    ...
                    cl = 26.0F;
                    string[] sServico = drRota["HoraProgramada"].ToString().Split(" "[0]);
                    int _iprimeiro = 0;
                    string _servrest = "";
                    ...
                    cl = cl + 186;
                    e.Graphics.DrawString(drRota["ExecutarServico"].ToString(), ft, Brushes.Black, cl, ln);
                    cl = cl + 230;
                    e.Graphics.DrawString(drRota["NomeFantasiaCliente"].ToString(), ft, Brushes.Black, cl, ln);
                    cl = cl + 200;
                    e.Graphics.DrawString(drRota["DestinoFinal"].ToString(), ft, Brushes.Black, cl, ln);
                    cl = cl + 200;
                    e.Graphics.DrawString(drRota["Observacao"].ToString(), ft, Brushes.Black, cl, ln);

                    ln = ln + 20F;
                    e.Graphics.DrawString("Endereço: ".ToUpper(), ft, Brushes.Black, 26, ln);                    
                    e.Graphics.DrawString(drRota["CidadeBairroEndereco"].ToString().Split("|"[0])[0], ft, Brushes.Black, 150, ln);
                    ln = ln + 12F;
                    e.Graphics.DrawString("Ponto de Referência: ".ToUpper(), ft, Brushes.Black, 26, ln);
                    string PR_semLineFeed = "";
                    if (drRota["CidadeBairroEndereco"].ToString() != "")
                        PR_semLineFeed = drRota["CidadeBairroEndereco"].ToString().Split("|"[0])[1].Replace("\n\r", "").Replace("\n", "").Replace("\r", "").Replace("\t", "");
                    ...
                    {
                        string _cdCliente = drRota["CodigoCliente"].ToString();
                        ...
                        if (_dtContDataReajuste.Rows.Count > 0)
                        {
                            ...
                            DataRow[] drr;
                            if (drRota["CodigoResiduo"].ToString() == "")
                                drr = _dtDadosResiduos.Select("CodigoResiduo = " + drRota["CodigoResiduo"].ToString());
                            else
                                drr = _dtDadosResiduos.Select("DescricaoReduzidaResiduo = '" + drRota[12].ToString() + "'");
                            ln = ln + 20F;
                            if (drr.Length > 0)
