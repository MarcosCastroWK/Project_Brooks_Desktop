...
{
    public partial class RelatorioMovimentacaoResiduosPorDestinoFinalDivergentes : System.Web.UI.Page
    {
                                   ...
                                   decimal _t1 = 0)
        {
            ...
            if (sExisteCodigoIBAMA == "" && chkIBAMA.Checked)
            ...
            else if (chkMostrarCDFe.Checked)
            {
                ...
                if (pEhTotalMTRe && _t1 == _qtCDFe)
                    AddRow(_table, _row, _cell, l_qt.ToString("N2"), iColWidth[13], true);
                else if (l_qt != _qtCDFe || pEhDiferenteNaLinha)
        ...
        private void Relatorio()
        {
            ...
            AddRow(_table, _row, _cell, "Relatório de movimentação de resíduos por destino final - Divergentes - Período de: " + Convert.ToDateTime(Data1.Data).ToString("dd/MM/yyyy") + " a " + Convert.ToDateTime(Data2.Data).ToString("dd/MM/yyyy"), 860, false, true);
            ...
            if (_dt.Rows.Count > 0)
            {
                ...
                if (chkPeso.Checked)
                {
                    ...
                    foreach (DataRow _dr in _dt.Rows)
                    {
                        ...
                        if (_dr["MTRe"].ToString() == "2306051859")
                        {
                        ...
                        if (_qtMtrs > 1)
                        {
                            ...
                            if (Math.Round(tMtrIbama, 2) == Math.Round(_qtCDFeConvertido, 2))
                            {
                                foreach (DataRow _dr2 in _dt.Select(_f1, ""))
                                {
                                    _dr2["DataDescarga"] = Convert.ToDateTime("01/01/0100");
                                }
                            }
                        }
                        else if (_qtMtrs == 1)
                        {
                            ...
                            if (Math.Round(tMtrIbama, 2) == Math.Round(_qtCDFeConvertido, 2))
                            {
                                _dr["DataDescarga"] = Convert.ToDateTime("01/01/0100");
                            }
                        }
                    }
                }
                ...
                for (int _i = 0; _i < _dt.Rows.Count; _i++)
                {
                    ...
                    if (dr["DataDescarga"].ToString() != "01/01/0100 00:00:00") // não mostra cdf-e emitida e ou pesos iguais
                    {
                        ...
                        if (dr["MTRe"].ToString() == "2305109924") // está no dia 11/05/23 em diante
                        {
                            // teste 
                            _vlSbTl = _vlSbTl + 0;
                        }
                        ...
                        if (_t1 != _t2 || dr["NumeroCDFe"].ToString() == "" || dr["NumeroCDFe"].ToString() == "0" || _cnpj_cpf != "" || sExisteCodigoIBAMA != dr["CodigoIBAMA"].ToString() || dr["Placas"].ToString().Replace(" ", "") != dr["PlacasNaCDFe"].ToString())
                        {
                            ...
                            else if (!chkCNPJ.Checked && !chkIBAMA.Checked && !chkCDFe.Checked && !chkPlacas.Checked && chkPeso.Checked)
                            {
                                ...
                                if (_MTReAnterior != (dr["MTRe"].ToString() + dr["CodigoIBAMA"].ToString()))
                                {
                                    if (_regsMTReIguais > 1 && _regsMTReAnteriorIguais >= 1 && l_qtSbTlPorMTRe != _t2)
                                    {
        ...
        {
            if (!chkCDFe.Checked && !chkCNPJ.Checked && !chkIBAMA.Checked && !chkPeso.Checked && !chkPlacas.Checked)
            {
                lblMensagem.Text = "Nenhum divergente selecionado!";
            }
            else
            {
                lblLinhas.Text = "";
        ...
        {
            string sx = "\n\n";
            sx = sx + "  Relatório de movimentação de resíduos por destino final - Divergentes - Período de: " + Convert.ToDateTime(Data1.Data).ToString("dd/MM/yyyy") + " a " + Convert.ToDateTime(Data2.Data).ToString("dd/MM/yyyy");
                    ...
                    try
                    {
                        ...
                        if (_MTReAnterior != (_dr["MTRe"].ToString() + _dr["CodigoIBAMA"].ToString()))
                        {
                            ...
                            if (_regsMTReIguais > 1) // && _regsMTReAnteriorIguais <= 1)
                            {
                                ...
                                if (_tt1 != _t2)
                                {
                                    font = new PdfSharp.Drawing.XFont("Courier New", 6.2, PdfSharp.Drawing.XFontStyleEx.Bold);
