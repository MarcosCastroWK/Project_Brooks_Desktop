...
namespace SILC.Web.forms
{
    public partial class DTR : System.Web.UI.Page
    {
        ...
        protected void Salvar_Click(object sender, EventArgs e)
        {
            ...
            if (Salvar.Text == "Salvar") // Confirma inclusão e alteração de linha
            {
                ...
                foreach (GridViewRow _gdr in Grade.Rows)
                {
                    ...
                    oDTR.DataSaida = ((DATA)_gdr.Cells[6].FindControl("datDataSaida")).Data;
        ...
        protected void Grade_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowIndex >= 0)
            {
                DATA _datDataSaida = (DATA)e.Row.Cells[5].FindControl("datDataSaida");
            ...
            if (Session["dtArmazenados"] != null && _dt.Rows.Count == 0)
            {
                try
                {
                    ...
                    foreach (GridViewRow gvr in Grade.Rows)
                    {
                        ...
                        _data = (DATA)gvr.Cells[6].FindControl("datDataSaida");
        ...
        protected void btnMTRImprimir_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow gvr in Grade.Rows)
            {
                ...
                _ResiduoDataSaidaLocalEntrega = _DescricaoResiduo + "-" + ((DATA)gvr.Cells[5].FindControl("datDataSaida")).Data + "-" + ((DropDownList)gvr.Cells[6].FindControl("ddlDestinoFinal")).Text;
                if (((DATA)gvr.Cells[5].FindControl("datDataSaida")).Data != "" && ((DropDownList)gvr.Cells[6].FindControl("ddlDestinoFinal")).Text != "" &&
            ...
            if (_EncontrouItemsParaImprimir)
            {
                ...
                foreach (GridViewRow gvr in Grade.Rows)
                {
                    ...
                    _CodigoResiduo = Convert.ToInt32(gvr.Cells[8].Text.Split("-"[0])[2]);
                    _DescricaoResiduo = oResiduoDado.PegaDescricao(_CodigoResiduo);
                    if (_ResiduoDataSaidaLocalEntrega == (_DescricaoResiduo + "-" + ((DATA)gvr.Cells[5].FindControl("datDataSaida")).Data + "-" + ((DropDownList)gvr.Cells[6].FindControl("ddlDestinoFinal")).Text))
                    {
                        ...
                        _dtRelacao.Rows[_dtRelacao.Rows.Count - 1]["DataSaida"] = geral.DataFormatada(((DATA)gvr.Cells[5].FindControl("datDataSaida")).Data);
            ...
            foreach (GridViewRow gvr in Grade.Rows)
            {
                    ...
                    ((TextBox)gvr.Cells[10].FindControl("txtImprimido")).Text == "I")
                {
                    ...
                    oLancamentoMTRDados.SalvarDestinoDataDescarga(_NumeroLancamento, Convert.ToInt32(_NumeroMTR), _CodigoResiduo,
                                                                  ((DATA)gvr.Cells[5].FindControl("datDataSaida")).Data,
