...
namespace LibSILC
{
    public class clsLancamentoMTRDados
    {
        ...
        public DataTable PreencheDTAterroSanitario(clsLancamentoMTR pLancamentoMTR, string pDataInicial = "", string pDataFinal = "", int pCodigoDestinoFinal = 0, int pCodigoCliente = 0, bool pAgrupadoPorMTRe = true, bool pMostrarMTReSemTicket = false)
        {
            ...
            s = s + "       (select sum(TotalPeso) from AterroSanitario where NumeroTicket = s.NumeroTicket and CodigoResiduo=999) as TotalPeso, sum(lmtr.Quantidade) as PesoIndividual, l.CodigoCliente, c.Nome, c.CNPJ_CPF, 0 as TotalGrupo, lmtr.NumeroMTRFatima as MTRe \n";
            ...
            if (pAgrupadoPorMTRe)
            {
                ...
                //s = s+"order by cast(NumeroTicket as unsigned) asc, Data asc, lmtr.NumeroMTRFatima asc, TotalPeso desc \n"; // não estava ordenando
        ...
        public string RetornaNumeroMTRe(int pNumeroLancamento, int pCodigoResiduo)
        {
            string sRet = "";
            try
            {
