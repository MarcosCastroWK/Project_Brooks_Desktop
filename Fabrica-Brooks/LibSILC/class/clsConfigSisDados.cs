...
namespace LibSILC
{
    public class clsConfigSisDados
	{
        ...
        public clsConfigSis PegaDados(clsConfigSis pConfigSis, int pCodigo)
        {
            try
            {
                ConectaBanco();
                s = "";
                s = s + "select Codigo, Campo, Ativo \n"; 
                s = s + "from   ConfigSis \n";
                if (pCodigo > 0)
                {
                ...
                if (l_ds.Tables.Count > 0)
                {
                    ...
                    if (l_dt.Rows[0]["Codigo"].ToString() != "")
                        pConfigSis.Codigo = Convert.ToInt32(l_dt.Rows[0]["Codigo"]);
                    pConfigSis.Campo = l_dt.Rows[0]["Campo"].ToString();
                    if (l_dt.Rows[0]["Ativo"].ToString() != "")
                        pConfigSis.Ativo = Convert.ToInt32(l_dt.Rows[0]["Ativo"]);
                }
                return pConfigSis;
            }
            catch (Exception ex)
            {
                return new clsConfigSis();
            }
            finally
            {
                DesconectaBanco();
        ...
        public DataTable PegaDados(clsConfigSis pConfigSis, int pCodigo, bool pUltimoRegistro)
        {
            try
            {
                ConectaBanco();
        ...
        public void Inserir(clsConfigSis pConfigSis, int pCodigo = 0)
        {
            try
            {
                s = "";
                s = s + "insert into ConfigSis \n";
                s = s + "( \n";
                if (pCodigo > 0)
                    s = s + "  Codigo, \n";
                s = s + "  Campo, Ativo \n"; 
                ...
                s = s + " '" + pConfigSis.Campo + "', \n";
                if (pConfigSis.Ativo > 0)
                    s = s + " " + pConfigSis.Ativo.ToString().Replace(",", ".") + " \n";
                else
                    s = s + "0 \n";
                s = s + ")"; 
                ConectaBanco();
        ...
        public string Alterar(clsConfigSis pConfigSis, int pCodigo)
        {
            ConectaBanco();
            MySqlCommand command = oDB.MySqlConnect.CreateCommand();
            MySqlTransaction transaction;
            ...
            {
                s = "";
                s = s + "update ConfigSis \n";
                
                s = s + "set Campo = '" + pConfigSis.Campo + "', \n";

                if (pConfigSis.Ativo > 0)
                    s = s + " Ativo = " + pConfigSis.Ativo.ToString().Replace(",", ".") + " \n";
                else
                    s = s + " Ativo = 0 \n";
                s = s + "where Codigo = " + pCodigo + " \n";        
                command.CommandText = s;
