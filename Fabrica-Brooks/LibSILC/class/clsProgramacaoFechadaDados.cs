...
namespace LibSILC
{
    public class clsProgramacaoFechadaDados
	{
        ...
        public DataTable MontaProgramacao(string pDataFutura, string pUsuario, bool pEncerramento = false)
        {
            ...
            s = s + "    where  (r.DataProgramada = '" + Convert.ToDateTime(pDataFutura).ToString("yyyy-MM-dd") + "' \n"; 
            s = s + "            or r.AnoMesDia = '" + Convert.ToDateTime(pDataFutura).ToString("yyyyMMdd") + "') \n";     
            s = s + "    and    (r.StatusCor = '8421631' or r.StatusCor = '8427929' or r.StatusCor = '064444' or r.StatusCor = '15000000') \n";
            s = s + "    group  by Hora, CodigoCliente, NomeFantasia  order by Sequencial, CodigoCliente \n";
            s = s + "   ) x \n";

            // quando coloco está condição NÃO aparece como antecipação, tomar cuidado. Aparece uma nova programação automática, mesmo depois de fechada.
            //s = s + "where  (select Count(*) \n";
            //s = s + "        from   ProgramacaoDiariaServicos \n";
            //s = s + "        where  StatusCor = '16761024' and CodigoCliente = x.CodigoCliente and Hora = x.Hora) = 0 \n";
            ...
            foreach (DataRow _drMensal in l_ds.Tables[0].Rows)
            {
                ...
                foreach (string strDia in _DiasColetas)
                {
                    if (Convert.ToInt32(_drMensal["CodigoCliente"]) == 1609)
            ...
            foreach (DataRow dr in dtFutura.Rows)
            {
                ...
                // aqui é horrível - à lentindão demais
                // edson test
                existeReprogramacaoFechada = oProgramacaoDados.ExisteReprogramacaoFechada(dr["Hora"].ToString(), Convert.ToInt32(dr["CodigoCliente"]),
                                                                                          Convert.ToInt32(dr["CodigoResiduo"]), dr["ExecutarServico"].ToString());
                if (existeReprogramacaoFechada)
                {
                    dr["StatusCor"] = geral.RetornaCodigoCor("Azul");
                }
                // fim edson test
            }
            return dtFutura;
